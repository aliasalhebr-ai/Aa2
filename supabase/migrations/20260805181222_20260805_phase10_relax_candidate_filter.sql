-- Relax candidate filter: if no items exist, match by type compatibility alone
-- Also match by shared sub_sector as fallback
...
BEGIN
  SELECT
    ...
    o.attributes, o.sub_sector_id
  INTO
    v_source_sector_id, v_source_type, v_source_status, v_source_city, v_source_timing,
    v_source_price, v_source_attrs, v_source_sub_sector_id
  ...
  SELECT EXISTS(SELECT 1 FROM opportunity_items WHERE opportunity_id = p_source_opportunity_id AND is_active = true)
  INTO v_source_has_items;

  RETURN QUERY
  WITH candidates AS (
    SELECT o.id, o.opportunity_type, o.city, o.opportunity_timing, o.sub_sector_id,
    ...
    WHERE o.sector_id = v_source_sector_id
      ...
      AND (
        -- If source has items, require shared plant OR shared sub_sector
        (v_source_has_items AND (
          EXISTS (
          ...
          OR (v_source_sub_sector_id IS NOT NULL AND o.sub_sector_id = v_source_sub_sector_id)
          OR EXISTS (SELECT 1 FROM opportunity_items cand_oi WHERE cand_oi.opportunity_id = o.id AND cand_oi.is_active = true)
        ))
        OR
        -- If source has no items, match by type compatibility + shared sub_sector or any candidate
        (NOT v_source_has_items AND (
          (v_source_sub_sector_id IS NOT NULL AND o.sub_sector_id = v_source_sub_sector_id)
          OR TRUE -- fallback: any compatible type in same sector
        ))
        OR
