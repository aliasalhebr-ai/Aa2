/*
# Phase 5 Final: Hardening Partnership Engine

## 1. RPC Changes
- Reject partnership without partnership_profile (must have partnership_type)
- Validate lead_entity_id ownership (must own the publisher entity)
- Add numeric validations:
  ...
  - project_sites > 0
  - total_quantity > 0
- Reject offer/demand with partnership_roles (not just profile)

## 2. UNIQUE Constraint
- partnership_roles: UNIQUE(opportunity_id, role_key) — one role per type per opportunity

## 3. RLS Hardening on partnership_role_catalog
- Revoke INSERT/UPDATE/DELETE from anon and authenticated
- Keep SELECT for anon + authenticated (active rows only)
- Only service_role / postgres can write (admin management)

## 4. Grant Restrictions
- partnership_role_catalog: remove write grants from anon + authenticated
*/

-- ═══════════════════════════════════════════════════════════════
-- 1. UNIQUE constraint on partnership_roles
-- ═══════════════════════════════════════════════════════════════

CREATE UNIQUE INDEX IF NOT EXISTS idx_partnership_roles_opp_role_key
  ON partnership_roles(opportunity_id, role_key);

-- ═══════════════════════════════════════════════════════════════
-- 2. Revoke write grants on partnership_role_catalog from anon/authenticated
-- ═══════════════════════════════════════════════════════════════

REVOKE INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES ON partnership_role_catalog FROM anon;
REVOKE INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES ON partnership_role_catalog FROM authenticated;
...
BEGIN
  ...
  IF p_operation_type = 'partnership' THEN
    -- partnership_profile is REQUIRED and must have partnership_type
    IF p_partnership_profile IS NULL
       OR jsonb_typeof(p_partnership_profile) <> 'object'
       OR p_partnership_profile = '{}'::jsonb THEN
      RAISE EXCEPTION 'VALIDATION_ERROR: ملف الشراكة مطلوب لفرص الشراكة';
    END IF;
    ...
    -- lead_entity_id must be owned by the user
    v_lead_entity_id := NULLIF(p_partnership_profile->>'lead_entity_id', '')::uuid;
    IF v_lead_entity_id IS NOT NULL THEN
      IF NOT EXISTS (
        SELECT 1 FROM publisher_entities
        WHERE id = v_lead_entity_id
          AND owner_user_id = v_user_id
          AND is_active = true
      ) THEN
        RAISE EXCEPTION 'AUTH_ERROR: الجهة القائدة يجب أن تكون جهة تملك إدارتها';
      END IF;
    END IF;

    -- Numeric validations on profile
    v_num_val := NULLIF(p_partnership_profile->>'project_value', '')::numeric;
    IF v_num_val IS NOT NULL AND v_num_val < 0 THEN
      RAISE EXCEPTION 'VALIDATION_ERROR: القيمة التقديرية للمشروع يجب أن تكون أكبر من أو تساوي صفر';
    END IF;

    v_num_val := NULLIF(p_partnership_profile->>'total_quantity', '')::numeric;
    IF v_num_val IS NOT NULL AND v_num_val <= 0 THEN
      RAISE EXCEPTION 'VALIDATION_ERROR: إجمالي الكمية يجب أن يكون أكبر من صفر';
    END IF;

    v_int_val := NULLIF(p_partnership_profile->>'project_phases', '')::integer;
    IF v_int_val IS NOT NULL AND v_int_val <= 0 THEN
      RAISE EXCEPTION 'VALIDATION_ERROR: عدد المراحل يجب أن يكون أكبر من صفر';
    END IF;

    v_int_val := NULLIF(p_partnership_profile->>'project_sites', '')::integer;
    IF v_int_val IS NOT NULL AND v_int_val <= 0 THEN
      RAISE EXCEPTION 'VALIDATION_ERROR: عدد المواقع يجب أن يكون أكبر من صفر';
    END IF;
    ...
    IF p_partnership_roles IS NOT NULL AND jsonb_typeof(p_partnership_roles) = 'array' THEN
      FOR v_role IN SELECT * FROM jsonb_array_elements(p_partnership_roles) LOOP
        ...
        -- Numeric validations on roles
        v_int_val := NULLIF(v_role->>'required_count', '')::integer;
        IF v_int_val IS NOT NULL AND v_int_val <= 0 THEN
          RAISE EXCEPTION 'VALIDATION_ERROR: العدد المطلوب للدور يجب أن يكون أكبر من صفر';
        END IF;

        v_num_val := NULLIF(v_role->>'required_quantity', '')::numeric;
        IF v_num_val IS NOT NULL AND v_num_val <= 0 THEN
          RAISE EXCEPTION 'VALIDATION_ERROR: الكمية المطلوبة للدور يجب أن تكون أكبر من صفر';
        END IF;

        v_num_val := NULLIF(v_role->>'minimum_capacity', '')::numeric;
        IF v_num_val IS NOT NULL AND v_num_val < 0 THEN
          RAISE EXCEPTION 'VALIDATION_ERROR: الحد الأدنى للطاقة يجب أن يكون أكبر من أو يساوي صفر';
        END IF;
      END LOOP;
    END IF;
  END IF;
  ...
  -- ── 11. Prevent partnership profile/roles for non-partnership types ──
  IF p_operation_type != 'partnership' THEN
    ...
    END IF;
    IF p_partnership_roles IS NOT NULL
       AND jsonb_typeof(p_partnership_roles) = 'array'
       AND jsonb_array_length(p_partnership_roles) > 0 THEN
      RAISE EXCEPTION 'VALIDATION_ERROR: لا يمكن ربط أدوار شراكة بفرصة ليست من نوع partnership';
    END IF;
  END IF;
  ...
  -- ── 14. Insert partnership profile + roles ──
  IF p_operation_type = 'partnership' THEN
    INSERT INTO partnership_opportunity_profiles (
      ...
      v_opportunity_id,
      v_partnership_type,
      COALESCE(v_lead_entity_id, p_publisher_entity_id),
