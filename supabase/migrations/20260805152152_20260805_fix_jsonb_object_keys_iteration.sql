/*
# Fix jsonb_object_keys iteration in create_v2_opportunity

The FOR loop used `SELECT key FROM jsonb_object_keys(v_item)` but
jsonb_object_keys returns a setof text directly, not a column named "key".
Fix: use `SELECT * FROM jsonb_object_keys(v_item)` instead.
*/
...
-- Re-grant permissions (function was recreated)
